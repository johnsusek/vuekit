...
import { VueKitConstructor, VueKitNodeProps } from '../types/VueKit';
...
export function getCandidateConstructors(elementClass: NSView, vnodeProps?: VueKitNodeProps) {
...
export function getConstructor(className: string, vnodeProps?: VueKitNodeProps) {
  ...
  if (constructor.args.length > 0) {
    ...
    constructor.name += constructor.pieces.reduce((acc, curr) => acc += curr[0].toUpperCase() + curr.toLowerCase().slice(1), '');
  }
...
export function setNodeValue(node: VueKitNode, key: string, value: any, fromPatch = false) {
  ...
  if (typeof objectToSet[setter] === 'function') {
    // console.info(`Using setter ${setter} on ${node.view}`);
...
export function getPropValues(requestedProps: any, props: VNodeProps) {
  let values: any = {};
